...
export function DecisionsPage() {
  ...
  return <>
    ...
    {decisions.length ? <div className="entity-grid">{decisions.map((decision) => <EntityCard key={decision.id} title={decision.title} meta={decision.category} excerpt={decision.question} icon={decision.status === "Open" ? <CircleHelp /> : <CircleCheckBig />} badges={<Badge tone={decision.status === "Open" ? "warning" : "success"}>{decision.status}</Badge>} onOpen={() => setSelection({ collection: "decisions", id: decision.id })} onEdit={() => openEdit("decisions", decision.id)} onArchive={() => requestArchive("decisions", decision.id)} />)}</div> : <Card><EmptyState icon={<CircleHelp />} title="No decisions match" description="Adjust the current filters or add a research decision with an explicit status." /></Card>}
