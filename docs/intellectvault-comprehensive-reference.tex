...
\title{IntellectVault Comprehensive Implementation Reference}
...
The authoritative IntellectVault path is \codepath{intellectVault/users/{{uid}}}. Database rules require the authenticated UID to equal the path UID for reads and writes. The current architecture is therefore a private owner workspace, not a shared collaborative vault.
...
This distinction matters: derived text in IntellectVault is not automatically AI-generated. The Mindset Builder is deterministic while Finder, Repurposer and Mixer are Gemini-backed.
...
The owner can export a local JSON snapshot of the current workspace. The page also displays the required IntellectVault Firebase rule as an operational reminder. Import/migration adapters are not implemented as a corresponding general restore workflow in the current inspected code.
