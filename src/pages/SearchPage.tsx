...
export function SearchPage() {
  ...
  return <>
    <PageHeader eyebrow="Global retrieval" title="Search" description="Search across active hierarchy, prompts, prompt history, mindsets, preferences, global versions, and decisions." />
    ...
    {query ? (results.length ? <div className="search-results">{results.map(({ kind, record }) => <button key={`${kind}:${record.id}`} onClick={() => setSelection({ collection: kind, id: record.id })}><Badge>{kind === "globalCommits" ? "global version" : kind === "promptVersions" ? "prompt version" : kind}</Badge><div><strong>{recordTitle(kind, record)}</strong><p>{JSON.stringify(record).replace(/[{}\[\]"]/g, " ").slice(0, 220)}</p></div></button>)}</div> : <Card><EmptyState icon={<Search />} title="No matching records" description={`No active records contain “${query}”.`} /></Card>) : <Card><EmptyState icon={<Search />} title="Search your vault" description="Enter a title, purpose, prompt phrase, preference, commit identifier, or decision question." /></Card>}
