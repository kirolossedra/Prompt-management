  ...
  return (
    <div className="versions-page">
      ...
      <section className="global-version-hero">
        <div><span className="eyebrow">Current vault</span><h2>Ready for a baseline whenever you decide.</h2><p>A Global Version captures prompts, prompt histories, attached files, prompt relationships, mindsets, preferences, endeavors, tasks, decisions, and profile state at one instant.</p></div>
        <div className="snapshot-counts"><span><strong>{activeRecords(data.prompts).length}</strong><small>Prompts</small></span><span><strong>{activeRecords(data.promptVersions).length}</strong><small>Local versions</small></span><span><strong>{activeRecords(data.mindsets).length}</strong><small>Mindsets</small></span><span><strong>{activeRecords(data.promptAttachments).length}</strong><small>Files</small></span><span><strong>{activeRecords(data.promptRelations).length}</strong><small>Links</small></span><span><strong>{activeRecords(data.preferences).length}</strong><small>Preferences</small></span></div>
      </section>
