...
export function AuthPage() {
  ...
  return <main className="auth-page">
    <motion.section className="auth-product" initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
      <div className="auth-brand"><span>IV</span><strong>IntellectVault</strong></div>
      ...
      <div className="auth-preview" aria-hidden>
        <div className="auth-preview__sidebar"><span className="mini-brand">IV</span><i /><i /><i /><i /></div>
    ...
    <motion.section className="auth-panel" initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: .08 }}>
      <div className="auth-panel__mobile-brand"><span>IV</span><strong>IntellectVault</strong></div>
      ...
      <form className="auth-form" onSubmit={submit}>
        {mode === "signup" ? <><FormField label="Your name" required>{(props) => <input {...props} name="displayName" autoComplete="name" required />}</FormField><FormField label="Workspace name" required>{(props) => <input {...props} name="workspaceName" placeholder="My IntellectVault" required />}</FormField></> : null}
        ...
        <Button variant="primary" size="lg" type="submit" loading={loading}>{mode === "signin" ? "Open IntellectVault" : "Create private vault"}</Button>
      </form>
